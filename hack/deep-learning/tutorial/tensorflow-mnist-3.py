...
def model(X, w1, w2, w_o, p_keep_conv, p_keep_hidden):
    ...
    with tf.name_scope("layer2"):
        ...
        l2 = tf.nn.max_pool(l2a, ksize=[1, 2, 2, 1],              # l2 shape=(?, 7, 7, 64)
                            strides=[1, 2, 2, 1], padding='SAME')
        l2 = tf.reshape(l2, [-1, w_o.get_shape().as_list()[0]])    # reshape to (?, 32)
...
w2 = init_weights([5, 5, 32, 64])     # 5x5x32 conv, 32 outputs
w_o = init_weights([64 * 7 * 7, 10])         # FC 32 * 7 * 7 inputs, 10 outputs (labels)
...
py_x = model(X, w1, w2, w_o, p_keep_conv, p_keep_hidden)
